"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   WALK-FORWARD BACKTEST — LONG + SHORT 15m — versione19                    ║
║   Laboratorio Sperimentale Controllato (NON ottimizzazione)                ║
║   Event-Driven Architecture + Pattern Confirmation + ATR Anchored          ║
╚══════════════════════════════════════════════════════════════════════════════╝

FILOSOFIA v19: correzione bug + ribilanciamento exit per aumentare EV per trade.

CAMBIAMENTI rispetto a v18:

  BUG FIX
  • [FIX] run_screening: condizione ridondante `idx < 479 or idx < 72`
    corretta in `idx < 479` (il ramo `or idx < 72` era sempre falso)
  • [FIX] calc_adx: eliminata duplicazione codice, ora chiama
    calc_adx_with_di e restituisce solo la colonna "adx"

  CONFIG — EXIT (ribilanciamento EV per trade)
  • breakeven_buffer:      0.005 → 0.010  (+1.0% vs +0.5%)
    Razionale: +0.5% genera +$0.35 su $100 slot, a malapena le fees.
    Con +1.0% il breakeven vale $0.85 netto e ha senso come safety net.
  • trailing_activation:   0.040 → 0.025  (+2.5% vs +4.0%)
    Razionale: con 0.04 solo 2/29 trade raggiungevano il trailing.
    Con 0.025 più trade vengono catturati in trailing, riducendo breakeven.
  • trailing_distance:     0.015 → 0.012
    Più stretto per preservare più profitto sul trailing.
  • sl_atr_max_distance:   0.120 → 0.060  (ceiling SL dal 12% al 6%)
    Razionale: trade come 0G/USDC con SL al 10.25% sono cigni neri
    inaccettabili su slot da $100. Cap a 6% = max $6 di perdita per trade.

  CONFIG — SCREENER (più frequenza)
  • runner_score_min:      40 → 30
    Razionale: 29 trade in 365gg non è statisticamente significativo.
    Con soglia più bassa si punta a 60-100 trade/anno.
  • runner_momentum_pct:   0.020 → 0.015
    Soglia momentum abbassata in coerenza con score_min.

  SHORT — invariato (abilitato, simulato)
  ⚠️  SHORT su Binance Spot non è eseguibile in produzione.
      Abilitato solo per misurare la distribuzione statistica.
      In v20: separare su futures o disabilitare definitivamente.

CONFRONTO PRE-RUN:
  v18 (15m, 365gg): PF 0.43, Return -3.25%, Trade 29, WR 75.9%
  v19 target:       PF > 0.8, Trade > 60, mantenere WR > 60%
"""
...
CONFIG = {
    ...
    # — Filtro attività body-based v18 —
    ...
    # — Breakeven v19 (trigger ±1.5%, target ±1.0%) —
    # v18: buffer era 0.005 → +$0.35 netto su $100, a malapena le fees
    # v19: buffer 0.010 → +$0.85 netto, ha senso come safety net reale
    ...
    # — Trailing v19 (activation abbassata da 4% a 2.5%) —
    # v18: activation 0.040 → solo 2/29 trade lo raggiungevano
    # v19: activation 0.025 → più trade catturati in trailing
    "trailing_activation":  0.025,          # v19: 0.040 → 0.025
    "trailing_distance":    0.012,          # v19: 0.015 → 0.012
    ...
    "sl_atr_max_distance":          0.06,   # v19: 0.12 → 0.06 (cap cigni neri)
...
os.makedirs("results_wf_v19", exist_ok=True)
...
def calc_adx(df: pd.DataFrame, period: int = 14) -> pd.Series:
    """Restituisce solo la serie ADX. v19: delega a calc_adx_with_di (no duplicazione)."""
...
def run_screening(all_data: dict, cutoff_ts: pd.Timestamp,
    ...
    for sym, dfs in all_data.items():
        try:
            ...
            # v19 FIX: era `idx < 479 or idx < 72` — il ramo `or idx < 72`
            # era sempre ridondante (sempre True se idx < 479).
            # Corretta in singola soglia warmup 1h (20 giorni × 24h).
            if idx < 479:
                continue
...
def precompute_signals(all_data: dict, cfg: dict) -> dict:
    print("\n  Pre-calcolo segnali (v19 TF15m + ATR anchored)...")
    signals = {}
...
class Position:
    """
    Posizione v19:
      - breakeven_buffer: 0.010 (era 0.005)
      - trailing_activation: 0.025 (era 0.040)
      - trailing_distance: 0.012 (era 0.015)
      - sl_atr_max_distance: 0.06 (era 0.12)
      - Ordering intrabar: 1.SL 2.Timeout 3.Breakeven 4.TP1 5.TP2 6.Trailing
      - Modello OHLC pessimistico
    """
    ...
    def __init__(self, symbol: str, direction: str,
        ...
        # ── Trailing v19: activation 2.5%, distance 1.2% ───────────────
    ...
    def check_exit_ohlc(self, candle_open: float, candle_high: float,
        ...
        """
        Ordering intrabar deterministico:
          1. SL   2. Timeout   3. Breakeven   4. TP1   5. TP2   6. Trailing
        Modello pessimistico: SL prima di TP se ambigui sulla stessa candela.
        """
        cfg = self.cfg

        # ── 1. SL HIT ──────────────────────────────────────────────────
        if self.sl_enabled:
            sl_hit = (candle_low  <= self.sl_price if self.direction == "long"
                      else candle_high >= self.sl_price)
            if sl_hit:
        ...
        if not self.breakeven_done:
            if self.direction == "long":
                trigger_price = self.entry_price * (1 + cfg["breakeven_trigger"])
                triggered     = candle_high >= trigger_price
            else:
                trigger_price = self.entry_price * (1 - cfg["breakeven_trigger"])
                triggered     = candle_low <= trigger_price
            if triggered:
                self.sl_price       = self.breakeven_price
                self.breakeven_done = True
                be_hit = (candle_low  <= self.sl_price if self.direction == "long"
                          else candle_high >= self.sl_price)
                if be_hit:
                    self._close(self.sl_price, ts, "breakeven_hit")
                    return True
        ...
        # ── 6. TRAILING ACTIVATION (v19: a +2.5%) ──────────────────────
        if not self.trailing_active:
            if self.direction == "long":
                if candle_high >= self.entry_price * (1 + cfg["trailing_activation"]):
                    self.trailing_active = True
                    self.best_price = max(self.best_price, candle_high)
            else:
                if candle_low <= self.entry_price * (1 - cfg["trailing_activation"]):
                    self.trailing_active = True
                    self.best_price = min(self.best_price, candle_low)

        # ── 7. TRAILING UPDATE + CHECK ──────────────────────────────────
        if self.trailing_active:
            ...
            if self.trailing_stop is not None:
                trail_hit = (candle_low  <= self.trailing_stop if self.direction == "long"
                             else candle_high >= self.trailing_stop)
                if trail_hit:
    ...
    def realized_pnl(self, commission_pct: float) -> float:
        ...
        # Partial closes a target price fisso (TP hit = exit esattamente al target)
        if self.tp1_done:
        ...
        # Residuo liquidato al prezzo di exit effettivo
...
def run_walkforward(all_data: dict, signals: dict,
    ...
    """Walk-forward v19 — Event-driven, anti-lookahead."""
    all_indices = set()
    ...
    for ts in tqdm(all_ts, desc="  Walk-Forward v19", unit="candela"):
...
def plot_results(equity_curve: list, trade_log: list, stats: dict, days: int):
    ...
    plt.tight_layout()
    out = f"results_wf_v19/equity_wf_{days}gg.png"
...
def print_summary(stats: dict, trade_log: list, days: int):
    cap_op = CONFIG["capital_total"] * (1 - CONFIG["reserve_pct"])
    print(f"\n{'═'*70}")
    print(f"{'RIEPILOGO v19 — LONG+SHORT — ' + str(days) + 'gg':^70}")
    print(f"{'═'*70}")
    if not stats:
        print("  Nessun trade.")
        return
    ...
    if trade_log:
        ...
        # ── BREAKDOWN per SL_distance (verifica cap 6%) ──────────────────
        sl_trades = df_t[df_t["exit_reason"] == "SL_atr"]
        if len(sl_trades) > 0:
            print(f"\n  ── SL_ATR DISTRIBUTION (v19 cap 6%) ─────")
...
def main():
    ...
    print(f"\n{'═'*65}")
    print(f"  WALK-FORWARD BACKTEST — LONG+SHORT 15m — versione19")
    print(f"  Fix bug + ribilanciamento exit (breakeven/trailing/SL)")
    print(f"{'═'*65}\n")

    print("  v19 vs v18 delta:")
    ...
    if trade_log:
        path_t = f"results_wf_v19/trades_wf_{days}gg.csv"
        pd.DataFrame(trade_log).to_csv(path_t, index=False)
        print(f"  [CSV] Trade: {path_t} ({len(trade_log)} trade)")

    if screening_log:
        path_s = f"results_wf_v19/screening_log_{days}gg.csv"
    ...
    if equity_curve:
        ...
        df_eq.to_csv(f"results_wf_v19/equity_wf_{days}gg.csv", index=False)

    plot_results(equity_curve, trade_log, stats, days)
    print_summary(stats, trade_log, days)

    print(f"\n✔  Walk-Forward v19 completato.")
    print(f"   Output → ./results_wf_v19/\n")
