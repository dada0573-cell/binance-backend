"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   WALK-FORWARD BACKTEST — LONG + SHORT 15m — versione20                    ║
║   Laboratorio Sperimentale Controllato (NON ottimizzazione)                ║
║   Event-Driven Architecture + Pattern Confirmation + ATR Anchored          ║
╚══════════════════════════════════════════════════════════════════════════════╝

FILOSOFIA v20: fix bug ordering exit + ripristino trailing activation a 3.5%.

CAMBIAMENTI rispetto a v19:

  BUG FIX — ordering intrabar critico
  • [FIX] check_exit_ohlc step 1: quando trailing_active=True il controllo SL
    veniva eseguito prima del trailing (step 7), causando uscite a sl_price
    (breakeven) invece che a trailing_stop (prezzo migliore).
    Prova: SSV/USDC e XVG/USDC uscivano "breakeven_hit" (+0.85) in v19
    invece di "trailing_stop" (+2.54/+2.58) come in v18.
    Fix: step 1 salta il controllo SL se trailing_active=True.
    Ragionamento corretto: trailing_stop >= breakeven_price >= sl_price
    per costruzione (update_trailing_stop usa max(..., breakeven_price)).
    Se candle_low <= sl_price, allora candle_low <= trailing_stop pure,
    quindi step 7 lo cattura al prezzo corretto "trailing_stop".

  CONFIG — TRAILING (ripristino activation)
  • trailing_activation:   0.025 → 0.035
    Razionale: con il bug fix, il problema SSV/XVG era nel codice, non
    nel parametro. Ripristinato a 0.035 (intermedio tra v18 0.040 e v19
    0.025) per un equilibrio tra copertura e noise avoidance.
    Con il bug fix attivo, ogni trailing_stop ora esce al prezzo corretto.

  INVARIATI rispetto a v19
  • breakeven_buffer:      0.010  (+1.0% target)
  • trailing_distance:     0.012
  • sl_atr_max_distance:   0.060  (cap 6%)
  • runner_score_min:      30
  • runner_momentum_pct:   0.015
  • [FIX] idx < 479 (no ridondanza)
  • [FIX] calc_adx delega a calc_adx_with_di
...
CONFRONTO:
  ...
  v19: PF 0.79, Return -1.04%, Trade 29, WR 79.3%
  v20 target: PF > 1.0, trailing_stop recupera i trade persi dal bug
"""
...
CONFIG = {
    ...
    # — Trailing v20 —
    # v19 abbassava a 0.025 ma il bug ordering causava uscite breakeven
    # premature. Con il bug fix ripristinato a 0.035 (intermedio).
    "trailing_activation":  0.035,          # v20: 0.025 → 0.035
    "trailing_distance":    0.012,          # invariato
...
os.makedirs("results_wf_v20", exist_ok=True)
...
def precompute_signals(all_data: dict, cfg: dict) -> dict:
    print("\n  Pre-calcolo segnali (v20 TF15m + ATR anchored)...")
    signals = {}
...
class Position:
    """
    Posizione v20:
      - BUG FIX: step 1 SL skipped when trailing_active=True
        (trailing handles all exits when active, preventing premature
        breakeven_hit instead of trailing_stop)
      - trailing_activation: 0.035 (v19: 0.025, ripristino dopo bug fix)
      - breakeven_buffer: 0.010
      - trailing_distance: 0.012
      - sl_atr_max_distance: 0.06
      - Ordering intrabar: 1.SL(*) 2.Timeout 3.Breakeven 4.TP1 5.TP2 6.Trailing activation 7.Trailing update+check
        (*) step 1 skipped when trailing_active=True
    """
    ...
    def check_exit_ohlc(self, candle_open: float, candle_high: float,
        ...
        """
        Ordering intrabar deterministico v20:
          1. SL   (skipped when trailing_active=True)
          2. Timeout
          3. Breakeven
          4. TP1
          5. TP2
          6. Trailing activation
          7. Trailing update + check

        BUG FIX v20: quando trailing_active=True, lo step 1 veniva eseguito
        prima dello step 7, causando uscite a sl_price (peggio) invece che
        a trailing_stop (meglio). Fix: skip step 1 se trailing_active.
        Invariante garantita: trailing_stop >= breakeven_price >= sl_price
        per costruzione (update_trailing_stop usa max/min con breakeven_price).
        """
        cfg = self.cfg

        # ── 1. SL HIT ──────────────────────────────────────────────────
        # v20 FIX: skip quando trailing_active=True.
        # trailing_stop >= sl_price per invariante, quindi step 7 copre tutto.
        if self.sl_enabled and not self.trailing_active:
        ...
        if not self.breakeven_done:
            ...
            if triggered:
                ...
                if be_hit and not self.trailing_active:
...
def run_walkforward(all_data: dict, signals: dict,
    ...
    for ts in tqdm(all_ts, desc="  Walk-Forward v20", unit="candela"):
...
def plot_results(equity_curve: list, trade_log: list, stats: dict, days: int):
    ...
    plt.tight_layout()
    out = f"results_wf_v20/equity_wf_{days}gg.png"
...
def print_summary(stats: dict, trade_log: list, days: int):
    cap_op = CONFIG["capital_total"] * (1 - CONFIG["reserve_pct"])
    print(f"\n{'═'*70}")
    print(f"{'RIEPILOGO v20 — LONG+SHORT — ' + str(days) + 'gg':^70}")
    print(f"{'═'*70}")
    if not stats:
        print("  Nessun trade.")
        return
...
def main():
    ...
    print(f"\n{'═'*65}")
    print(f"  WALK-FORWARD BACKTEST — LONG+SHORT 15m — versione20")
    print(f"  Bug fix ordering exit + trailing activation 3.5%")
    print(f"{'═'*65}\n")

    print("  v20 vs v19 delta:")
    print(f"   trailing_activation: 0.025 → {cfg['trailing_activation']:.3f}  (ripristino dopo bug fix)")
    print(f"   BUG FIX: step 1 SL skip quando trailing_active=True")
    print(f"\n  v20 vs v18 delta (totale):")
    ...
    if trade_log:
        path_t = f"results_wf_v20/trades_wf_{days}gg.csv"
        pd.DataFrame(trade_log).to_csv(path_t, index=False)
        print(f"  [CSV] Trade: {path_t} ({len(trade_log)} trade)")

    if screening_log:
        path_s = f"results_wf_v20/screening_log_{days}gg.csv"
    ...
    if equity_curve:
        ...
        df_eq.to_csv(f"results_wf_v20/equity_wf_{days}gg.csv", index=False)

    plot_results(equity_curve, trade_log, stats, days)
    print_summary(stats, trade_log, days)

    print(f"\n✔  Walk-Forward v20 completato.")
    print(f"   Output → ./results_wf_v20/\n")
