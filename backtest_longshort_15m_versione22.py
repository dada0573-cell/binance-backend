"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   WALK-FORWARD BACKTEST — LONG + SHORT 15m — versione22                    ║
║   Laboratorio Sperimentale Controllato (NON ottimizzazione)                ║
║   Event-Driven Architecture + Pattern Confirmation + ATR Anchored          ║
╚══════════════════════════════════════════════════════════════════════════════╝

FILOSOFIA v22: aumento frequenza screener (opzione B) su base v21 corretta.

CAMBIAMENTI rispetto a v21:

  CONFIG — SCREENER (opzione B: aumenta frequenza)
  • screening_interval_hours: 4 → 2
    Razionale: con intervallo 4h si perdono finestre di opportunità nelle
    sessioni più attive. Con 2h lo screener gira 12 volte/giorno invece di 6,
    raddoppiando la copertura temporale.
  • screening_top_n:          5 → 8
    Razionale: basket più ampio cattura più candidati per slot disponibile,
    riducendo i momenti in cui non ci sono segnali validi.
...
CONFRONTO:
  ...
  v22 target: Trade > 60, mantenere PF > 1.0
"""
...
os.makedirs("results_wf_v22", exist_ok=True)
...
def precompute_signals(all_data: dict, cfg: dict) -> dict:
    print("\n  Pre-calcolo segnali (v22 TF15m + ATR anchored)...")
    signals = {}
...
class Position:
    """
    Posizione v22 — two-pass OHLC model (invariato da v21):
...
def run_walkforward(all_data: dict, signals: dict,
    ...
    """Walk-forward v22 — Event-driven, anti-lookahead."""
    all_indices = set()
    ...
    for ts in tqdm(all_ts, desc="  Walk-Forward v22", unit="candela"):
...
def plot_results(equity_curve: list, trade_log: list, stats: dict, days: int):
    ...
    plt.tight_layout()
    out = f"results_wf_v22/equity_wf_{days}gg.png"
...
def print_summary(stats: dict, trade_log: list, days: int):
    cap_op = CONFIG["capital_total"] * (1 - CONFIG["reserve_pct"])
    print(f"\n{'═'*70}")
    print(f"{'RIEPILOGO v22 — LONG+SHORT —' + str(days) + 'gg':^70}")
    print(f"{'═'*70}")
    if not stats:
        print("  Nessun trade.")
        return
...
def main():
    ...
    print(f"\n{'═'*65}")
    print(f"  WALK-FORWARD BACKTEST — LONG+SHORT 15m — versione22")
    print(f"  Opzione B: aumenta frequenza screener (interval 2h, top_n 8, score_min 25)")
    print(f"{'═'*65}\n")

    print("  v22 vs v21 delta (opzione B):")
    ...
    if trade_log:
        path_t = f"results_wf_v22/trades_wf_{days}gg.csv"
        pd.DataFrame(trade_log).to_csv(path_t, index=False)
        print(f"  [CSV] Trade: {path_t} ({len(trade_log)} trade)")

    if screening_log:
        path_s = f"results_wf_v22/screening_log_{days}gg.csv"
    ...
    if equity_curve:
        ...
        df_eq.to_csv(f"results_wf_v22/equity_wf_{days}gg.csv", index=False)

    plot_results(equity_curve, trade_log, stats, days)
    print_summary(stats, trade_log, days)

    print(f"\n✔  Walk-Forward v22 completato.")
    print(f"   Output → ./results_wf_v22/")
