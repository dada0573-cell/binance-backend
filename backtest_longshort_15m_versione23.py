"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   WALK-FORWARD BACKTEST — LONG + SHORT 15m — versione23                    ║
║   Laboratorio Sperimentale Controllato (NON ottimizzazione)                ║
║   Event-Driven Architecture + Pattern Confirmation + ATR Anchored          ║
╚══════════════════════════════════════════════════════════════════════════════╝

FILOSOFIA v23: filtro ATR all'entry (scarta coin troppo volatili) + SL short
più largo per dare più respiro alle posizioni SHORT.

CAMBIAMENTI rispetto a v22:

  CONFIG — SL SHORT
  • sl_atr_multiplier_short:  3.0 → 3.5
    Razionale: gli SHORT hanno meno corsa rispetto ai LONG in un mercato
    tendenzialmente rialzista; dare più margine riduce le uscite premature
    per SL su spike momentanei.

  LOGICA ENTRY — FILTRO ATR (nuovo in v23)
  • Se il SL naturale (atr_ratio × multiplier, prima del capping) supera
    sl_atr_max_distance (6%), la coin viene scartata all'entry anziché
    entrare con SL cappato al 6%.
    Razionale: le coin che richiederebbero un SL > 6% per stare fuori dal
    rumore (es. GPS, XTZ, 0G in v22) sono troppo volatili e generano le
    perdite più grandi. Meglio non entrare.

  (invariato rispetto a v22)
  • screening_interval_hours: 4 → 2
  • screening_top_n:          5 → 8
  • runner_score_min:         30 → 25
    Razionale: 29 trade in 365gg non è statisticamente significativo.
    Soglia abbassata per puntare a 60-100 trade/anno.
    La qualità è mantenuta dagli altri filtri (ADX, RSI, volume spike, DI).

  INVARIATI rispetto a v21
  • Two-pass OHLC model (definitivo)
...
CONFRONTO:
  ...
  v20: PF 0.77, Return -1.11%, Trade 29, WR 75.9%
  v21: PF 1.45, Return +2.16%, Trade 29, WR 75.9%  (two-pass OHLC fix)
  v23 target: ridurre perdite SL su coin iper-volatili, migliorare SHORT PnL
"""
...
CONFIG = {
    ...
    "sl_atr_multiplier_short":      3.5,   # v23: 3.0 → 3.5
...
os.makedirs("results_wf_v23", exist_ok=True)
...
def precompute_signals(all_data: dict, cfg: dict) -> dict:
    print("\n  Pre-calcolo segnali (v23 TF15m + ATR anchored)...")
    signals = {}
...
class Position:
    """
    Posizione v23 — two-pass OHLC model (invariato da v21):
    ...
    def check_exit_ohlc(self, candle_open: float, candle_high: float,
        ...
        """
        Two-pass OHLC model (invariato da v21).
...
def run_walkforward(all_data: dict, signals: dict,
    ...
    """Walk-forward v23 — Event-driven, anti-lookahead."""
    all_indices = set()
    ...
    for ts in tqdm(all_ts, desc="  Walk-Forward v23", unit="candela"):
        ...
        for sym, ps in confirmed_list:
            ...
            # v23: scarta entry se SL naturale supera il cap (coin troppo volatile)
            atr_ratio_check = (atr_anchor / entry_price
                               if (atr_anchor is not None and atr_anchor > 0
                                   and entry_price > 0)
                               else cfg["sl_atr_min_distance"])
            mult_check = (cfg["sl_atr_multiplier_long"] if ps.direction == "long"
                          else cfg["sl_atr_multiplier_short"])
            if atr_ratio_check * mult_check > cfg["sl_atr_max_distance"]:
                del pending_signals[sym]
                continue
...
def plot_results(equity_curve: list, trade_log: list, stats: dict, days: int):
    ...
    plt.tight_layout()
    out = f"results_wf_v23/equity_wf_{days}gg.png"
...
def print_summary(stats: dict, trade_log: list, days: int):
    cap_op = CONFIG["capital_total"] * (1 - CONFIG["reserve_pct"])
    print(f"\n{'═'*70}")
    print(f"{'RIEPILOGO v23 — LONG+SHORT —' + str(days) + 'gg':^70}")
    print(f"{'═'*70}")
    if not stats:
        print("  Nessun trade.")
        return
...
def main():
    ...
    print(f"\n{'═'*65}")
    print(f"  WALK-FORWARD BACKTEST — LONG+SHORT 15m — versione23")
    print(f"  v23: filtro ATR entry + sl_atr_multiplier_short 3.0→3.5")
    print(f"{'═'*65}\n")

    print("  v23 vs v22 delta:")
    print(f"   sl_atr_multiplier_short: 3.0 → {cfg['sl_atr_multiplier_short']}")
    print(f"   filtro ATR entry: scarta coin con SL naturale > sl_atr_max_distance")
    print(f"\n  v23 vs v21 delta (opzione B invariata):")
    print(f"   screening_interval_hours: 4 → {cfg['screening_interval_hours']}")
    print(f"   screening_top_n:          5 → {cfg['screening_top_n']}")
    print(f"   runner_score_min:         30 → {cfg['runner_score_min']}")
    print(f"\n  delta cumulativo vs v18:")
    ...
    if trade_log:
        path_t = f"results_wf_v23/trades_wf_{days}gg.csv"
        pd.DataFrame(trade_log).to_csv(path_t, index=False)
        print(f"  [CSV] Trade: {path_t} ({len(trade_log)} trade)")

    if screening_log:
        path_s = f"results_wf_v23/screening_log_{days}gg.csv"
    ...
    if equity_curve:
        ...
        df_eq.to_csv(f"results_wf_v23/equity_wf_{days}gg.csv", index=False)

    plot_results(equity_curve, trade_log, stats, days)
    print_summary(stats, trade_log, days)

    print(f"\n✔  Walk-Forward v23 completato.")
    print(f"   Output → ./results_wf_v23/")
    print(f"   Opzione B: interval={cfg['screening_interval_hours']}h, top_n={cfg['screening_top_n']}, score_min={cfg['runner_score_min']}\n")
