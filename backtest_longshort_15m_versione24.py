"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   WALK-FORWARD BACKTEST — LONG ONLY  15m — versione24                     ║
║   Laboratorio Sperimentale Controllato (NON ottimizzazione)                ║
║   Event-Driven Architecture + Pattern Confirmation + ATR Anchored          ║
╚══════════════════════════════════════════════════════════════════════════════╝

FILOSOFIA v24: fix strutturali di realismo (OHLC stop-first, entry open T+1,
SHORT disabilitato su spot) + SL più largo + trailing progressivo.

CAMBIAMENTI rispetto a v22:

  BUG FIX — OHLC model: stop-first conservative (v24)
  • PASS 0 (nuovo): controlla SL e trailing_stop PRE-candela prima di qualsiasi
    attivazione. Le nuove attivazioni (breakeven, trailing, TP) aggiornano lo
    stato ma i nuovi stop valgono solo dalla candela successiva.
    Razionale: nel v22 un'attivazione intra-candela poteva "salvare" una
    posizione che avrebbe dovuto essere fermata dallo stop pre-esistente
    (es. H=104 attiva trailing → L=93 chiude a trailing_stop=102.75 invece
    di SL=94). Il fix è conservativo e riduce il bias ottimistico.

  BUG FIX — Entry price: open della candela T+1 (non close di conferma)
  • Quando il pattern di conferma è rilevato alla close di T, l'entry avviene
    alla open di T+1 (candela successiva). Prima entrava alla close di T, che
    è impossibile in produzione (la close è nota solo a fine candela).

  BUG FIX — End-of-test close price
  • Usa signals[sym][index <= final_ts]["close"].iloc[-1] invece di iloc[-1]
    assoluto, per evitare prezzi fuori periodo.

  BUG FIX — Errori silenziati in precompute_signals
  • Conta e stampa il numero di pair saltati per eccezione.

  CONFIG — SHORT disabilitato (spot realistico)
  • short_enabled: True → False
    Razionale: SHORT su Binance Spot non è eseguibile. I risultati SHORT erano
    pura simulazione teorica. Disabilitando si misura solo quello realizzabile.

  CONFIG — SL più largo + filtro entry
  • sl_atr_multiplier_long:  4.0 → 4.5  (più margine prima dello stop)
  • sl_atr_max_distance:     6%  → 7%   (cap più alto → meno capping artificiale)
  • sl_atr_entry_filter=True: scarta entry se SL naturale > 7% (coin iper-volatili
    come GPS/XTZ/0G che in v22 generavano le 4 perdite peggiori cappate al 6%)
    Razionale: con moltiplicatore 4.5 e cap 7%, monete con ATR/price > 1.56%
    vengono filtrate. Le monete normali (ATR/price ~1%) hanno SL naturale ~4.5%,
    ben dentro il cap.

  CONFIG — Trailing progressivo
  • trailing_distance: 1.2% → 1.0%  (trail più stretto = più profitto bloccato)
  • trailing_distance_tight = 0.7%   (nuovo: usato dopo trailing_tighten_at)
  • trailing_tighten_at     = 5.0%   (nuovo: quando gain > 5%, trail si stringe)
    Razionale: i runner grandi escono troppo presto. Trail stretto > 5% blocca
    più profitto sulle rare ma decisive operazioni da doppia cifra.

  INVARIATI rispetto a v22
  • screening_interval_hours: 2, top_n: 8, score_min: 25
  • breakeven_trigger: 1.5%, breakeven_buffer: 1.0%
  • trailing_activation: 2.5%
  • tp1: 3% (30%), tp2: 7% (30%)
  • timeout: 24h

CONFRONTO:
  v21: PF 1.45, Return +2.16%, Trade  29, WR 75.9%  (two-pass OHLC fix)
  v22: PF 1.28, Return +3.34%, Trade  67, WR 76.1%  (screener +frequenza)
  v23: PF 1.19, Return +1.44%, Trade  47, WR 74.5%  (filtro ATR aggressivo)
  v24 target: correggere il bias OHLC + entry realistica, misurare impatto
"""
...
CONFIG = {
    ...
    # — Direzione —
    "long_enabled":         True,
    "short_enabled":        False,          # v24: disabilitato — spot non supporta short reale

    # — Screening bidirezionale —
    "screening_interval_hours": 2,          # v22: 4 → 2 (opzione B: più frequenza)
    "screening_top_n":          8,          # v22: 5 → 8
    ...
    "runner_score_min":         25,         # v22: 30 → 25 (opzione B: più frequenza)
    ...
    # — Trailing v24 — progressivo (stretto dopo trailing_tighten_at)
    "trailing_activation":      0.025,
    "trailing_distance":        0.010,      # v24: 1.2% → 1.0%
    "trailing_distance_tight":  0.007,      # v24: 0.7% dopo tighten_at
    "trailing_tighten_at":      0.050,      # v24: stringe il trail dopo +5% gain

    # — SL ATR-anchored —
    "sl_atr_period":                20,
    "sl_atr_anchor_lookback":       2,
    "sl_atr_multiplier_long":       4.5,    # v24: 4.0 → 4.5
    "sl_atr_multiplier_short":      3.5,    # (inutilizzato, short disabilitato)
    "sl_atr_min_distance":          0.02,
    "sl_atr_max_distance":          0.07,   # v24: 6% → 7%
    "sl_atr_entry_filter":          True,   # v24: scarta entry se SL naturale > max_distance
...
os.makedirs("results_wf_v24", exist_ok=True)
...
def precompute_signals(all_data: dict, cfg: dict) -> dict:
    print("\n  Pre-calcolo segnali (v24 TF15m + ATR anchored)...")
    signals  = {}
    n_errors = 0
    ...
    if n_errors:
        print(f"  ⚠️  Pair saltati per errore: {n_errors}")
...
class Position:
    """
    Posizione v24 — OHLC model stop-first conservative:
      PASS 0 (lato avverso): controlla stop PRE-candela prima di qualsiasi attivazione.
        0a. trailing_stop esistente   0b. SL/breakeven esistente
      PASS 1 (lato favorevole): attivazioni — i nuovi stop valgono dalla candela successiva.
        1a. breakeven trigger   1b. trailing activation   1c. trailing update
        1d. TP1 parziale        1e. TP2 parziale
      Timeout: solo se non trailing_active.
      Trailing progressivo: distance=1.0% fino a +5%, poi 0.7%.
    """
    ...
    def __init__(self, symbol: str, direction: str,
        ...
        self.trailing_distance_tight = cfg.get("trailing_distance_tight",
                                               cfg["trailing_distance"])
        self.trailing_tighten_at     = cfg.get("trailing_tighten_at", float("inf"))
    ...
    def update_trailing_stop(self):
        if not self.trailing_active:
            return
        cfg  = self.cfg
        gain = self.pnl_pct(self.best_price)
        dist = (self.trailing_distance_tight
                if gain >= self.trailing_tighten_at
                else cfg["trailing_distance"])
        if self.direction == "long":
            trail = self.best_price * (1 - dist)
    ...
    def check_exit_ohlc(self, candle_open: float, candle_high: float,
        ...
        """
        OHLC model stop-first conservative (v24).

        PASS 0 — stop PRE-candela (lato avverso):
          Controlla SL e trailing_stop calcolati nelle candele precedenti
          PRIMA di qualsiasi attivazione della candela corrente.
          Impedisce il bias ottimistico dove un'attivazione intra-candela
          (es. H attiva trailing) "salva" da uno stop che era già violato
          dal lato avverso (es. L < SL originale).

        PASS 1 — attivazioni (lato favorevole):
          Aggiorna stato (breakeven, trailing, TP). I nuovi stop (trailing_stop
          appena calcolato, SL spostato a breakeven) valgono dalla candela
          successiva — non vengono ricontrollati nella stessa candela.

        Timeout: solo se trailing non ancora attivo.
        """
        cfg = self.cfg

        # ══════════════════════════════════════════════════════════════
        # PASS 0 — STOP PRE-CANDELA (conservative / stop-first)
        # ══════════════════════════════════════════════════════════════

        # 0a. Trailing stop pre-candela
        if self.trailing_active and self.trailing_stop is not None:
            if self.direction == "long":
                if candle_low <= self.trailing_stop:
                    self._close(self.trailing_stop, ts, "trailing_stop")
                    return True
            else:
                if candle_high >= self.trailing_stop:
                    self._close(self.trailing_stop, ts, "trailing_stop")
                    return True

        # 0b. SL / breakeven pre-candela
        if self.sl_enabled:
            if self.direction == "long":
        ...
        # ══════════════════════════════════════════════════════════════
        # PASS 1 — ATTIVAZIONI (lato favorevole)
        # I nuovi stop calcolati qui valgono dalla candela successiva.
        # ══════════════════════════════════════════════════════════════
        ...
        # 1c. Trailing best_price update + trailing_stop (vale dalla prossima candela)
...
class PendingSignal:
    ...
    def __init__(self, symbol: str, direction: str,
        ...
        self.status          = "pending"
        self.entry_ts        = None   # impostato quando status → confirmed
...
def run_walkforward(all_data: dict, signals: dict,
    ...
    """Walk-forward v24 — Event-driven, anti-lookahead, entry a open(T+1)."""
    all_indices = set()
    ...
    for ts in tqdm(all_ts, desc="  Walk-Forward v24", unit="candela"):
        ...
        for sym in list(pending_syms):
            ...
            if pattern_ok:
                ps.status   = "confirmed"
                ps.entry_ts = ts + pd.Timedelta(minutes=candle_minutes)
        ...
        # Segnali confermati pronti per entry (entry_ts raggiunto)
        confirmed_list = [(sym, ps) for sym, ps in pending_signals.items()
                          if ps.status == "confirmed"
                          and ps.entry_ts is not None
                          and ts >= ps.entry_ts]
        ...
        for sym, ps in confirmed_list:
            ...
            row         = signals[sym].loc[ts]
            entry_price = float(row["open"])          # v24: open(T+1), non close(T)
            atr_anchor  = (float(row["atr_anchored"])
                           if not pd.isna(row.get("atr_anchored", None)) else None)

            # v24: filtro entry — scarta coin se SL naturale supera il cap
            if cfg.get("sl_atr_entry_filter", False) and entry_price > 0:
                atr_ratio_chk = (atr_anchor / entry_price
                                 if (atr_anchor is not None and atr_anchor > 0)
                                 else cfg["sl_atr_min_distance"])
                mult_chk = (cfg["sl_atr_multiplier_long"] if ps.direction == "long"
                            else cfg["sl_atr_multiplier_short"])
                if atr_ratio_chk * mult_chk > cfg["sl_atr_max_distance"]:
                    del pending_signals[sym]
                    continue
    ...
    for slot_id, pos in slots.items():
        if pos is not None and not pos.closed:
            ...
            if sym in signals:
                df_sym = signals[sym]
                df_sym_pre = df_sym[df_sym.index <= final_ts]
                final_price = float(df_sym_pre["close"].iloc[-1]) if len(df_sym_pre) else pos.entry_price
            else:
...
def plot_results(equity_curve: list, trade_log: list, stats: dict, days: int):
    ...
    plt.tight_layout()
    out = f"results_wf_v24/equity_wf_{days}gg.png"
...
def print_summary(stats: dict, trade_log: list, days: int):
    cap_op = CONFIG["capital_total"] * (1 - CONFIG["reserve_pct"])
    print(f"\n{'═'*70}")
    print(f"{'RIEPILOGO v24 — LONG+SHORT —' + str(days) + 'gg':^70}")
    print(f"{'═'*70}")
    if not stats:
        print("  Nessun trade.")
        return
...
def main():
    ...
    print(f"\n{'═'*65}")
    print(f"  WALK-FORWARD BACKTEST — LONG ONLY 15m — versione24")
    print(f"  Fix: stop-first OHLC + entry open(T+1) + SL largo + trailing progressivo")
    print(f"{'═'*65}\n")

    print("  v24 vs v22 delta (fix strutturali):")
    print(f"   short_enabled:           True  → {cfg['short_enabled']}")
    print(f"   sl_atr_multiplier_long:  4.0   → {cfg['sl_atr_multiplier_long']}")
    print(f"   sl_atr_max_distance:     0.06  → {cfg['sl_atr_max_distance']:.2f}")
    print(f"   sl_atr_entry_filter:     False → {cfg.get('sl_atr_entry_filter', False)}")
    print(f"   trailing_distance:       0.012 → {cfg['trailing_distance']:.3f}")
    print(f"   trailing_distance_tight: new   → {cfg.get('trailing_distance_tight', 0):.3f}  (dopo +{cfg.get('trailing_tighten_at',0)*100:.0f}%)")
    print(f"   OHLC model: two-pass → stop-first conservative")
    print(f"   Entry price: close(T) → open(T+1)")
    print()
    ...
    if trade_log:
        path_t = f"results_wf_v24/trades_wf_{days}gg.csv"
        pd.DataFrame(trade_log).to_csv(path_t, index=False)
        print(f"  [CSV] Trade: {path_t} ({len(trade_log)} trade)")

    if screening_log:
        path_s = f"results_wf_v24/screening_log_{days}gg.csv"
    ...
    if equity_curve:
        ...
        df_eq.to_csv(f"results_wf_v24/equity_wf_{days}gg.csv", index=False)

    plot_results(equity_curve, trade_log, stats, days)
    print_summary(stats, trade_log, days)

    print(f"\n✔  Walk-Forward v24 completato.")
    print(f"   Output → ./results_wf_v24/")
    print(f"   interval={cfg['screening_interval_hours']}h, top_n={cfg['screening_top_n']}, score_min={cfg['runner_score_min']}\n")
